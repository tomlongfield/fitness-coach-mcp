...
export const config = {
  ...
  // Optional — enables POST /health-relay/hrv (see lib/health-relay.js) when
  // set. Not required(): unlike everything else in this file, this is an
  // opt-in feature, and making it required would crash every existing
  // deployment at startup until they added a var for a route they may not
  // even want. The route itself 503s if this is unset.
  healthRelaySecret: process.env.HEALTH_RELAY_SECRET || null,
  // IANA zone used to resolve "today"/"N days ago" (see lib/date-utils.js).
  // Defaults to UTC rather than the host machine's TZ so behavior doesn't
  // depend on how the deployment environment happens to be configured.
