...
// POST /api/health-data — accepts a single {value,type,date} object or an
// array of them; a `type` not matching one of SparkyFitness's fixed fields
// (weight, step, water, "Active Calories") becomes a custom measurement
// category, auto-created on first use — same mechanism getCustomMeasurementRange
// reads back from, confirmed against SparkyFitness's own server source.
export async function postHealthData(entries) {
  let res;
  try {
    res = await fetch(`${config.sparkyFitnessBaseUrl}/health-data`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${config.sparkyFitnessApiKey}`,
    ...
    if (err.name === 'TimeoutError') {
      throw new Error(`SparkyFitness did not respond within ${FETCH_TIMEOUT_MS / 1000}s posting to /health-data.`);
    }
    throw err;
  }
  const body = await res.json().catch(() => null);
  if (!res.ok) {
    throw new Error(`SparkyFitness returned ${res.status} posting to /health-data: ${JSON.stringify(body)}`);
  }
  return body;
}
