...
const VITALS_CATEGORIES = [
  ...
  // HRV deliberately isn't here — see get_hrv_samples. Every field above is
  // genuinely one-value-per-day; HRV_SDNN's category is configured for
  // multiple raw readings per day (frequency "All", not "Daily" — see
  // lib/health-relay.js), which this Map-per-date merge has no way to
...
// Raw per-reading, not merged to one-per-day like fetchVitalsEntries above —
// HRV_SDNN's category is configured for multiple entries a day (frequency
// "All"), and folding them into a daily figure here would just be
// re-inventing the averaging get_vitals_trend deliberately doesn't do for
// this metric. Sorted chronologically so a consumer doesn't have to.
function summarizeHrvSample(e) {
  return { timestamp: e.timestamp, value: Number(e.value) };
}

async function fetchHrvSamples(days) {
