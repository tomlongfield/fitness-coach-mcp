...
const VITALS_CATEGORIES = [
  ...
  // Case-sensitive, and NOT the same category "hrv" would resolve to —
  // SparkyFitness's own server source has separate display-name/unit
  // entries for HRV (RMSSD) vs HRV_SDNN (SDNN). Apple Health only ever
  // measures SDNN (HKQuantityTypeIdentifierHeartRateVariabilitySDNN); RMSSD
  // is a Garmin/Fitbit-style figure Apple-sourced accounts have no data
  // for. Fed by lib/health-relay.js, not a first-party SparkyFitness/Apple
  // Health sync path — single readings are noisy (many samples/day,
  // affected by posture, activity, time since eating) and there's no
  // established personal baseline for a new account, so treat a
  // multi-week trend against itself as meaningful, not any single day's
  // absolute value.
  ['HRV_SDNN', 'hrvSdnnMs'],
];
...
export function buildServer() {
  ...
  server.registerTool(
    'get_vitals_trend',
    {
      ...
      description:
        'Returns daily heart rate (resting, min/max/avg), respiratory rate, blood oxygen saturation, VO2 max, Apple Watch activity metrics (distance, exercise/stand minutes, floors climbed), and heart rate variability (hrvSdnnMs) from SparkyFitness over a recent window. All sourced from Apple Health via SparkyFitness custom-measurement categories, not its fixed check-in schema — see get_bodyweight_trend for weight/body-composition instead. hrvSdnnMs is inherently noisy day to day and a new account has no established personal baseline yet — read it as a multi-week trend against itself, not a meaningful single-day figure.',
