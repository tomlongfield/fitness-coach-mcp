...
/**
 * Reshapes Health Auto Export's fixed {data:{metrics:[{name,units,data}]}}
 * export shape into SparkyFitness's flat {value,type,date} /api/health-data
 * ingest shape — the two don't overlap at all (confirmed against both
 * systems' real source/docs), so nothing simpler than a relay was viable.
 ...
 * on top of that network restriction, not a substitute for it.
 */
export function registerHealthRelayRoutes(app) {
  app.post('/health-relay/hrv', async (req, res) => {
    ...
    if (!Array.isArray(metrics)) {
      ...
      res.status(400).json({ error: 'unexpected_payload', message: 'Expected body.data.metrics to be an array.' });
      return;
    }
    ...
    // First real run: hit with ?debug=1 to log the raw metric object and
    // confirm its actual field names before trusting the parsed output —
    // VALUE_FIELDS/DATE_FIELDS above are a best guess, not a confirmed shape.
    ...
    const entries = daily.map((d) => ({ value: d.value, type: 'HRV_SDNN', date: d.date }));
