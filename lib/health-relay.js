...
import { postHealthData, getCustomMeasurementRange } from './sparkyfitness-client.js';
...
// SparkyFitness's "All"-frequency categories have no dedup of their own —
// confirmed against its own upsert logic (unlike "Daily"/"Hourly", which
// look up an existing row first, "All"/"Unlimited" always INSERTs) and
// confirmed the hard way live: an overlapping pair of backfill runs left
// 269 exact duplicate rows across 158 timestamps, since cleaned up by
// hand. Anything posted through this route needs its own dedup, done here
// rather than relied on downstream.
async function filterOutExisting(entries) {
  if (!entries.length) return entries;
  const oldestMs = Math.min(...entries.map((e) => Date.parse(e.timestamp)));
  const spanDays = Math.ceil((Date.now() - oldestMs) / 86400000) + 1;
  const existing = await getCustomMeasurementRange('HRV_SDNN', spanDays);
  const existingTimestamps = new Set(existing.map((e) => Date.parse(e.timestamp)));
  return entries.filter((e) => !existingTimestamps.has(Date.parse(e.timestamp)));
}
...
// Shortcuts' JSON request-body editor has no way to set a field with a
// blank key (unlike older Shortcuts versions) or to drop a List variable
// into a field directly as a nested array — confirmed empirically across
// several live attempts, not guessed: a List-typed field builds its own
// empty list container instead of accepting the variable, and a
// Text-typed field converts a List variable to newline-joined,
// individually-JSON-stringified lines (effectively JSON Lines) rather
// than one JSON array. Both are handled below rather than fought further.
function parseReadingsField(readings) {
...
// send either one {value, timestamp?} reading or an array of them
// (optionally wrapped under a "readings" key — see parseReadingsField),
// already shaped the way it was measured. timestamp defaults to now if
// omitted.
function parseSimplePayload(body) {
  const raw = parseReadingsField(body?.readings) ?? body;
...
export function registerHealthRelayRoutes(app) {
  app.post('/health-relay/hrv', async (req, res) => {
    ...
    entries = await filterOutExisting(entries);
    const skipped = beforeDedup - entries.length;
    if (skipped) console.log(`[health-relay] skipped ${skipped} already-stored reading(s)`);

    const dryRun = req.query.dryRun === '1' || req.query.dryRun === 'true';
    if (dryRun) {
      res.status(200).json({ dryRun: true, skippedAsDuplicate: skipped, wouldPost: entries });
      return;
    }

    if (!entries.length) {
      res.status(200).json({ message: 'Every reading in this batch was already stored.', skippedAsDuplicate: skipped });
      return;
    }

    try {
      const sparkyFitnessResult = await postHealthData(entries);
      res.status(200).json({ posted: entries, skippedAsDuplicate: skipped, sparkyFitnessResult });
    } catch (err) {
